...
export const addToCart = (products: any, flag: any, prescription?: any, options?: {
    ...
    return axios
        ...
        .catch(err => {
            ...
            return addToLocalAndReturnSuccess().then((res) => {
                if (typeof window !== "undefined") window.dispatchEvent(new Event("cart-updated"));
                return res;
            });
        })
    ...
    return axios.get('/api/v1/cart', { params })
        .then(response => {
            ...
            if (d && d.success === true && Array.isArray(d.cart)) {
                // Merge API cart with localStorage items (in case add-to-cart fell back to local when API failed)
                const apiCartIds = new Set((d.cart || []).map((i: any) => String(i.cart_id ?? i.cart_item_id ?? i.id ?? "")));
                const localItems = getLocalCart().filter((local: any) => {
                    const lid = String(local.cart_id ?? "");
                    return lid && !apiCartIds.has(lid);
                });
                if (localItems.length > 0) {
                    console.log("getCart: merging", localItems.length, "local item(s) with API cart");
                    const mergedCart = [...(d.cart || []), ...localItems];
                    const merged = {
                        ...d,
                        cart: mergedCart,
                        total_items: mergedCart.length,
                    };
                    return { data: { status: true, ...merged } };
                }
...
/** Request backend to send order confirmation email (after payment). Idempotent. */
export const sendOrderConfirmationEmail = (order_id: string) => {
    return axios.post(`/api/v1/orders/${encodeURIComponent(order_id)}/send-confirmation-email`)
        .then(response => ({ data: response.data }))
        .catch(error => {
            const detail = error?.response?.data?.detail || error?.response?.data?.message || error?.message;
            console.error("[sendOrderConfirmationEmail]", detail || error);
            return { data: { success: false, message: typeof detail === 'string' ? detail : 'Request failed' } };
        });
}
