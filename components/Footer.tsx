...
export const Footer = () => {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    setMessage(null);
    try {
      const res = await axios.post("/api/v1/newsletter/subscribe", {
        email: email.trim(),
      });
      setMessage({
        type: "success",
        text: (res.data && (res.data as { message?: string }).message) || "Thanks for subscribing!",
      });
      setEmail("");
    } catch (err: unknown) {
      ...
      let msg: string;
      if (status === 404) {
        msg = "Newsletter endpoint not found. Is the backend running?";
      } else if (status === 503) {
        msg = "Service temporarily unavailable. Please try again later.";
      } else if (detail && typeof detail === "string") {
        msg = detail;
      } else if (ax.message === "Network Error" || !status) {
        msg = "Could not reach server. Please try again later.";
      } else {
              ...
              <div className="flex flex-col items-start">
                ...
                >
                  <input
                    ...
                    required
                    disabled={submitting}
                    className="bg-white border-none rounded-xl px-4 py-5 placeholder-black text-sm text-black font-bold flex-1 min-w-0 disabled:opacity-70"
                  />
                  <button
                    type="submit"
                    disabled={submitting}
                    className="bg-[#1F1F1F] text-white rounded-xl px-5 py-2.5 md:px-5 md:py-5 hover:bg-[#333] transition-colors whitespace-nowrap flex-shrink-0 disabled:opacity-70 disabled:cursor-not-allowed"
                  >
          ...
          <div className="mb-8">
            ...
            >
              <input
                ...
                required
                disabled={submitting}
                className="flex-1 bg-white border-none rounded-xl px-2 py-2 text-sm text-[#999] placeholder-[#999] disabled:opacity-70"
              />

              <button
                type="submit"
                disabled={submitting}
                className="bg-[#1F1F1F] text-white rounded-xl px-4 py-2 font-semibold hover:bg-[#333] transition-colors whitespace-nowrap disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {submitting ? "..." : "SUBSCRIBE"}
              </button>
            </form>
            {message && (
              <p className={`mt-2 text-sm text-center ${message.type === "success" ? "text-green-700" : "text-red-600"}`}>
                {message.text}
              </p>
            )}
          </div>
