...
interface PrescriptionData {
  prescriptionType?: string; // e.g. "Single Vision", "Progressive"
  prescriptionMethod?: string; // "manual", "upload", "photo"
  pd?: string;
  birthYear?: string;
...
export interface LensDetailsItem {
  lensType?: string;       // Category: "Blue Protect", "Clear"
  lensIndex?: string;      // "1.61 High Index"
...
const ProductDetailsFooter: React.FC<ProductDetailsFooterProps> = ({
  ...
  // Check if we have actual prescription details to show
  const hasPrescriptionDetails = prescriptionData && (prescriptionData.prescriptionType || prescriptionData.pd);
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6">
              ...
              {selectedColor && (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-[10px] font-medium text-gray-600">
                    {typeof selectedColor === 'string' ? '' : (selectedColor as ColorObject).frameColor || ''}
                  </span>
                </div>
              )}
          ...
          {hasPrescriptionDetails && (
            <div className="mt-4 bg-[#F3F0E7] rounded-3xl p-4 shadow-sm border border-black">
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-sm font-bold text-[#1F1F1F]">Prescription</h3>
                {prescriptionData?.prescriptionMethod && (
              ...
              <div className="space-y-2 text-xs text-gray-700">
                 <p><strong>Type:</strong> {prescriptionData?.prescriptionType || 'N/A'}</p>
                 
                 {/* PD Display */}
                 {prescriptionData?.pd ? (
                    <p><strong>PD:</strong> {prescriptionData.pd}</p>
                 ) : (
                    (prescriptionData?.od?.sph || prescriptionData?.os?.sph) && (
                       <p><strong>PD:</strong> Included in Rx</p>
                    )
                 )}

                 {/* Manual Entry Details */}
                 {prescriptionData?.prescriptionMethod === 'manual' && (prescriptionData?.od || prescriptionData?.os) && (
                    <div className="mt-2 grid grid-cols-2 gap-4 border-t border-gray-200 pt-2">
                       <div>
                         <p className="font-bold mb-1">Right (OD)</p>
                         <p>SPH: {prescriptionData.od?.sph || '-'}</p>
                         <p>CYL: {prescriptionData.od?.cyl || '-'}</p>
                         <p>AXIS: {prescriptionData.od?.axis || '-'}</p>
                       </div>
                       <div>
                         <p className="font-bold mb-1">Left (OS)</p>
                         <p>SPH: {prescriptionData.os?.sph || '-'}</p>
                         <p>CYL: {prescriptionData.os?.cyl || '-'}</p>
                         <p>AXIS: {prescriptionData.os?.axis || '-'}</p>
                       </div>
                    </div>
                 )}

                 {/* Photo/Upload Message */}
                 {(prescriptionData?.prescriptionMethod === 'upload' || prescriptionData?.prescriptionMethod === 'photo') && (
                    <p className="text-[#E94D37] font-medium pt-1">
                       *Image attached. We will verify details after checkout.
                    </p>
                 )}
              </div>
            </div>
          ...
          {/* Lens / Config card */}
          {(lensDetails || (prescriptionData && !hasPrescriptionDetails)) && (
            <div className="mt-4 bg-[#F3F0E7] rounded-3xl p-4 shadow-sm border border-black">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-[#F3F0E7] border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.05)] z-[80] font-sans pb-safe">
        <div className="flex flex-col items-center pt-3 pb-3 md:pb-0">
          ...
          {/* Stepper Indicator - Only on Mobile */}
          <div className="md:hidden flex items-center justify-center w-full max-w-[300px] mt-4 mb-2">
            ...
            {/* Step 2: Lens */}
            <div className="flex items-center gap-1.5 min-w-max">
              <div className="w-6 h-6 rounded-full bg-[#E94D37] text-white flex items-center justify-center text-[11px] font-bold">2</div>
            ...
            {/* Step 3: Check */}
            <div className="flex items-center gap-1.5 min-w-max">
              <div className="w-6 h-6 rounded-full border border-gray-400 text-gray-400 flex items-center justify-center text-[11px] font-bold">3</div>
