...
const ProductPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeProduct = async () => {
      ...
      if (id && initialProduct) saveProduct(id, initialProduct as any);
  ...
  // Persist product to session flow (per product) so downstream steps can restore it
  useEffect(() => {
    if (id && product) saveProduct(id, product as any);
  }, [id, product]);
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="block lg:hidden">
        ...
        >
          ...
          {/* MFit Button for Mobile - REPLACED ORANGE ICON WITH IMAGE */}
 ...
 {!capturedData && product && (
        ...
        : `left-1/2 -translate-x-1/2 ${vtoThumbHidden ? "bottom-full opacity-0 pointer-events-none" : "bottom-22 opacity-100"}`
          }`}
          aria-label="Try MFit - Virtual Try On"
        >
          {isMobile ? (
            // UPDATED: Replaced orange div/SVG with the image used on desktop
            <img 
...
<img src="/try-mfit-circle.svg" alt="MFit" width={80} height={60} />
            </div>
          )}
        </button>
      )}
